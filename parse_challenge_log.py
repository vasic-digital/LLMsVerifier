...
def parse_challenge_log(log_file):
    ...
    # Find all model verification completions
    model_pattern = r'Mandatory verification completed for model ([^:]+): .*?model_id.*?"([^"]+)".*?provider_id.*?"([^"]+)"'
    models = re.findall(model_pattern, log_content, re.DOTALL)
    
    print(f'Found {len(models)} models from regex')
    
    # Group by provider
    provider_models = {}
    for match in models:
        model_name, model_id, provider_id = match
