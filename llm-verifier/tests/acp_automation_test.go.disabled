...
func TestACPsAutomatedMonitoring(t *testing.T) {
	...
	t.Run("Threshold Detection", func(t *testing.T) {
		...
		for _, alert := range alerts {
			switch alert.Type {
			case "low_acp_score":
				if val, ok := alert.Value.(float64); ok && alert.ModelID == "claude-3-opus" && val < 0.7 {
					foundLowScoreAlert = true
				}
			case "high_error_rate":
				if val, ok := alert.Value.(float64); ok && alert.ModelID == "claude-3-opus" && val > 0.1 {
					foundHighErrorRateAlert = true
				}
			case "slow_performance":
				if val, ok := alert.Value.(time.Duration); ok && alert.ModelID == "claude-3-opus" && val > 5*time.Second {
