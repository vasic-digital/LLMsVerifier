...
func TestGetModelsFromConfig(t *testing.T) {
	...
	if models[0].ID != "model-1" {
		t.Errorf("Expected model ID 'model-1', got '%s'", models[0].ID)
	}
	
	if models[0].Name != "Test Model 1" {
		t.Errorf("Expected model name 'Test Model 1', got '%s'", models[0].Name)
	}
	
	if !models[0].SupportsBrotli {
		t.Error("Model should support brotli")
	}
	
	// Check second model
	if models[1].ID != "model-2" {
		t.Errorf("Expected model ID 'model-2', got '%s'", models[1].ID)
	}
	
	if models[1].MaxTokens != 8192 {
		t.Errorf("Expected max tokens 8192, got %d", models[1].MaxTokens)
	}
}
