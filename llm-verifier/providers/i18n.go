// Package-level i18n seam for the providers package per CONST-046.
//
// translator is a package-level variable so provider error-classification
// call sites can be migrated incrementally without threading a Translator
// argument through every ProviderError.Message assignment. Consumers that
...
// and events/i18n.go seams (rounds 316 / 319 / 326 / earlier) so the
...
// translator is the active i18n backend for the providers package. Tests in
...
// context and discards the error path — provider error messages have no
