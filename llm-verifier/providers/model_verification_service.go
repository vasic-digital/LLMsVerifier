...
func (mvs *ModelVerificationService) VerifyModel(ctx context.Context, model Model, providerClient *ProviderClient) (*ModelVerificationResult, error) {
	...
	// FIRST: Verify the model gives meaningful responses to basic prompts like "hello!"
	// This is a mandatory check - if the model cannot respond meaningfully, it's not verified
	meaningfulResult, err := mvs.codeVerificationSvc.VerifyMeaningfulResponse(
		ctx,
		model.ID,
		model.ProviderID,
	...
	// Check if meaningful response verification failed
	if err != nil || !meaningfulResult.HasMeaningfulResponse {
		mvs.logger.Error(fmt.Sprintf("Model %s failed meaningful response verification: %v", model.ID, err), map[string]interface{}{
			"model_id":     model.ID,
			"provider_id":  model.ProviderID,
			"error":        err,
			"has_response": meaningfulResult != nil && meaningfulResult.HasMeaningfulResponse,
		})
		result.VerificationStatus = "error"
		if meaningfulResult != nil {
			result.ErrorMessage = fmt.Sprintf("Meaningful response verification failed: %s (response length: %d)", meaningfulResult.ErrorMessage, meaningfulResult.ResponseLength)
		} else {
			result.ErrorMessage = fmt.Sprintf("Meaningful response verification failed: %v", err)
		}
	...
	// Save debate result for penalty application after code visibility scoring
	debateTestFailed := err != nil || !debateResult.HasMeaningfulResponse
	if debateTestFailed {
	...
	// Apply debate prompt penalties now that the score is set from code visibility
	if result.VerificationScore > 0.0 {
		if debateTestFailed {
			if result.VerificationScore > 0.5 {
				result.VerificationScore -= 0.3
				result.ErrorMessage = fmt.Sprintf("Slow on realistic prompts (%dms)", debateResult.ResponseTimeMs)
			}
		} else if debateResult.ResponseTimeMs > 30000 {
			penalty := 0.0
