// Package-level i18n seam for the monitoring package per CONST-046.
//
// translator is a package-level variable so health-check call sites can be
// migrated incrementally without threading a Translator argument through
// every ComponentHealth.Message assignment. Consumers that link this
...
//
// This mirrors the llmverifier/i18n.go and enhanced/i18n.go seams (rounds
// 316 / earlier) so the migration pattern is uniform across binary and
// library packages.
package monitoring
...
// translator is the active i18n backend for the monitoring package. Tests
...
// context and discards the error path — the health-status sites have no
