// Package-level i18n seam for the config package per CONST-046.
//
// translator is a package-level variable so validation call sites can be
// migrated incrementally without threading a Translator argument through
// every validate* helper signature. Consumers that link this package
...
// and enhanced/analytics/i18n.go seams (rounds 316 / 319 / 326 / round-29)
...
// translator is the active i18n backend for the config package. Tests in
// this package may swap it for a fakeConfigTranslator that returns
...
// context and discards the error path — validation messages have no
...
// previously emitted fmt.Sprintf / fmt.Printf with substitution
