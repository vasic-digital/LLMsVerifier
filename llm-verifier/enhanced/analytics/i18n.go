// Package-level i18n seam for the enhanced/analytics package per CONST-046.
//
// translator is a package-level variable so report-builder call sites can be
// migrated incrementally without threading a Translator argument through every
// ExecutiveSummary / DetailedReport / ChartData constructor. Consumers that
// link this package directly may override the variable at init time; the
...
// and events/i18n.go seams (rounds 316 / 319 / 326 / 328) so the migration
...
// and discards the error path — report labels have no graceful fallback
