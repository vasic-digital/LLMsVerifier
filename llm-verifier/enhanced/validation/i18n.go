// Package-level i18n seam for the enhanced/validation package per CONST-046.
//
// translator is a package-level variable so validation-gate call sites can be
// migrated incrementally without threading a Translator argument through every
// SyntaxValidator / SemanticValidator method signature. Consumers that link
// this package directly may override the variable at init time; the default
...
//
// This mirrors the llmverifier/i18n.go, enhanced/i18n.go, enhanced/analytics/i18n.go,
// monitoring/i18n.go and events/i18n.go seams (rounds 316 / 319 / 326 / 328 / 332)
// so the migration pattern stays uniform across binary and library packages.
package validation
...
// translator is the active i18n backend for the validation package. Tests in
// this package may swap it for a fakeValidationTranslator that returns
...
// and discards the error path — validation messages have no graceful fallback
// beyond returning the messageID, which is exactly what NoopTranslator does
