...
func (v *Verifier) writeModelReport(file *os.File, result VerificationResult) {
	...
	fmt.Fprintf(file, "- **%s**: %t\n", tr("report.field.rag"), result.FeatureDetection.RAG)
	fmt.Fprintf(file, "- **%s**: %t\n", tr("report.field.skills"), result.FeatureDetection.Skills)
	fmt.Fprintf(file, "- **%s**: %t\n", tr("report.field.plugins"), result.FeatureDetection.Plugins)
