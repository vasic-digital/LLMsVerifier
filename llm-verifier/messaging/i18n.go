// Package-level i18n seam for the messaging package per CONST-046.
//
// translator is a package-level variable so verification-event publishing
// call sites can be migrated incrementally without threading a Translator
// argument through every NewVerificationEvent title/message assignment.
...
// which lets tests assert sentinels rather than English literals
// (anti-bluff invariant per CONST-035 / Article XI §11.9).
//
// This mirrors the events/i18n.go, analytics/i18n.go, api/i18n.go and
// monitoring/i18n.go seams (rounds 316 / 319 / 326 / …) so the migration
// pattern stays uniform across binary and library packages.
package messaging
...
// translator is the active i18n backend for the messaging package. Tests in
...
// context and discards the error path — event titles have no graceful
