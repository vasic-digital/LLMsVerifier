...
func main() {
	...
	config := generateUltimateOpenCode(allModels, service)
...
func generateUltimateOpenCode(allModels map[string][]providers.Model, service *providers.ModelProviderService) map[string]interface{} {
	...
	for providerID, models := range allModels {
		...
		providerConfig[providerID] = map[string]interface{}{
			"models": modelMap,
		}
	}
