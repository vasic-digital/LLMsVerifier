...
// SMTP-notifier half of HXC-268, registered in the §11.4.135 standing
// regression-guard suite.
//
// # The defect
//
// EmailNotifier.sendEmail composed its smtp.SendMail address with
// fmt.Sprintf("%s:%d", en.smtpServer, en.smtpPort). For an IPv6 relay that is not
// a usable address: net.SplitHostPort — the parse every dialer performs — rejects
// it. Measured on the pre-fix artifact at port 587:
//
...
//
// So notifications to an IPv6 relay failed with a connection error that gave no
// hint the address itself was malformed.
//
...
// has no dialAddress() method at all — sendEmail composed the address inline —
// so the reproduction was performed with the original
// fmt.Sprintf("%s:%d", en.smtpServer, en.smtpPort) expression restored INSIDE
// the extracted helper. Same operands, same verbs, same call site, so the
...
// given the HelixAgent placeholder — a mail client must fail loudly on a bad
...
func TestHXC268DialAddressRED(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			// Synthetic credentials: fabricated fixtures, not a credential of any
			// real system, and never sent anywhere — dialAddress performs no I/O.
			en := NewEmailNotifier(tc.host, port, "synthetic-user", "synthetic-not-a-real-password",
				"noreply@example.invalid", []string{"ops@example.invalid"})
			got := en.dialAddress()
