#!/usr/bin/env bash
# llmsverifier_describe_challenge.sh — round-296 anti-bluff
# wrapper around the in-process Challenge runner
# (challenges/runner/main.go). Mirrors the LLMProvider round-292 /
# DocProcessor round-220 wrappers.
#
# Two-mode behaviour (CONST-050(A) paired-mutation; §1.1):
#
#   normal:    exits 0 only when the runner exits 0 (all 25
#              invariants pass). Any deviation FAILS.
#
#   mutate:    sets LLMSVERIFIER_MUTATE_RUNNER=1 which inverts
#              invariant 3 (validator.rejects_no_providers.*)
...
MODE="${1:-normal}"
echo "=== LLMsVerifier Describe Challenge (round-296) ==="
echo "  mode=${MODE}"
...
case "${MODE}" in
    normal)
        unset LLMSVERIFIER_MUTATE_RUNNER
        ...
        ;;
    mutate)
        export LLMSVERIFIER_MUTATE_RUNNER=1
        out="$(go run ./challenges/runner/ 2>&1)"
        rc=$?
        echo "${out}" | tail -15
        if [[ "${rc}" -eq 0 ]]; then
