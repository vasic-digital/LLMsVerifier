// Package-level i18n seam for the benchmark package per CONST-046.
//
// The root `llmsverifier` module does not yet ship a shared i18n package
// (the `digital.vasic.llmsverifier/pkg/i18n` package lives in the separate
// `llm-verifier/` module and is not importable from here without a module
// dependency). This file therefore declares a minimal, self-contained
// Translator interface plus a NoopTranslator default — identical in shape
// to the llmops seam (round 308+) and the llm-verifier seams (rounds 316 /
// 319 / 326 / 328 / 332) so the migration pattern stays uniform across all
// three modules.
//
// translator is a package-level variable so log / diff / description call
// sites can be migrated incrementally without threading a Translator
// argument through every constructor. Consumers that link this package
// directly may override the variable at init time; the default
...
// Translator is the minimal i18n surface every benchmark call site uses.
...
// translator is the active i18n backend for the benchmark package. Tests in
...
// context and discards the error path — log messages and descriptions
...
// previously emitted fmt.Sprintf / fmt.Errorf with substitution
...
// ICU {field}, etc.); NoopTranslator still returns the messageID verbatim
// so tests can assert the i18n-routed sentinel without coupling to a
// specific backend.
